...
async fn main() {
    ...
    if !is_prod { println!("Press Ctrl+C to stop."); }
...
async fn init_default_admin(db: &Database) {
    ...
    if db.get_admin_by_username(default_user).await.unwrap_or(None).is_some() { return; }
...
async fn handle_login(State(state): State<AppState>, Form(form): Form<LoginForm>) -> Response {
    ...
    if let Some(admin) = maybe_admin {
        if verify_password(&admin.password_hash, &form.password) {
            ...
            if state.db.create_admin_session(admin.id, &token).await.is_ok() {
                let mut headers = HeaderMap::new();
                let is_prod = env::var("APP_ENV").unwrap_or_default() == "production";
                let secure = if is_prod { "; Secure" } else { "" };
                let cookie = format!("admin_session={}; HttpOnly; SameSite=Strict; Path=/{}{}", token, secure, "");
...
async fn handle_logout(State(state): State<AppState>, headers: HeaderMap) -> impl IntoResponse {
    ...
    let mut headers = HeaderMap::new();
    headers.insert(header::SET_COOKIE, header::HeaderValue::from_static("admin_session=deleted; HttpOnly; SameSite=Strict; Path=/; Max-Age=0"));
    (headers, Redirect::to("/"))
}
...
async fn auth_me(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<AuthMeResponse>, StatusCode> {
    ...
    if admin.is_some() { Ok(Json(AuthMeResponse { is_admin: true })) }
...
async fn list_servers(State(state): State<AppState>) -> Result<Json<Vec<ServerApi>>, StatusCode> {
    ...
    for s in servers {
        ...
        res.push(ServerApi {
            id: s.id, name: s.name, address: s.address, port: s.port, created_at: s.created_at,
...
async fn create_server_json(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<CreateServerJson>) -> Result<Json<ServerApi>, StatusCode> {
    ...
    if body.port.unwrap_or(25565) < 1 || body.name.is_empty() { return Err(StatusCode::BAD_REQUEST); }

    let id = state.db.insert_server(&body.name, &body.address, body.port.unwrap_or(25565)).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let s = state.db.get_server_by_id(id).await.unwrap().unwrap();

    Ok(Json(ServerApi { id: s.id, name: s.name, address: s.address, port: s.port, created_at: s.created_at, last_online: false }))
}
...
async fn ping_and_store(State(state): State<AppState>, Path(id): Path<i64>) -> Result<Json<SimpleResponse>, StatusCode> {
    ping_one_server(&state, id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(SimpleResponse { success: true }))
}

async fn list_server_ping_history(State(state): State<AppState>, Path(id): Path<i64>) -> Result<Json<Vec<PingResult>>, StatusCode> {
    let mut res = state.db.list_ping_results_for_server(id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    res.reverse();
    Ok(Json(res))
}
...
async fn ping_all_servers_concurrently(state: &AppState) -> Result<(), ()> {
    let servers = state.db.list_servers().await.map_err(|e| eprintln!("Ping list error: {:?}", e))?;
    for s in servers {
        let st = state.clone();
        tokio::spawn(async move { let _ = ping_one_server(&st, s.id).await; });
    }
    Ok(())
}

async fn ping_one_server(state: &AppState, id: i64) -> Result<(), ()> {
    let s = match state.db.get_server_by_id(id).await { Ok(Some(v)) => v, _ => return Ok(()) };

    let mut stream = match TcpStream::connect((s.address.as_str(), s.port as u16)).await {
        Ok(s) => s,
        Err(_) => { let _ = state.db.insert_ping_result(s.id, false, None, None, None, None, None).await; return Ok(()); }
    };
    ...
    match ping(&mut stream, s.address.as_str(), s.port as u16).await {
        Ok(r) => {
            ...
            let _ = state.db.insert_ping_result(s.id, true, None, Some(r.online_players as i64), Some(r.max_players as i64), Some(r.version.as_str()), Some(desc.as_str())).await;
        },
        Err(_) => { let _ = state.db.insert_ping_result(s.id, false, None, None, None, None, None).await; }
    }
    Ok(())
}
...
fn hash_password(p: &str) -> String {
    ...
    Argon2::default().hash_password(p.as_bytes(), &SaltString::encode_b64(&salt).unwrap()).unwrap().to_string()
}
...
fn generate_session_token() -> String { let mut b = [0u8; 32]; OsRng.fill_bytes(&mut b); hex::encode(b) }
fn get_session_token_from_headers(h: &HeaderMap) -> Option<String> {
    h.get(header::COOKIE)?.to_str().ok()?.split(';').find_map(|s| s.trim().strip_prefix("admin_session=").map(String::from))
}
async fn get_admin_from_headers(state: &AppState, h: &HeaderMap) -> Result<AdminUser, StatusCode> {
    ...
    state.db.get_admin_by_session_token(&t).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?.ok_or(StatusCode::UNAUTHORIZED)
}

async fn shutdown_signal() {
    let ctrl_c = async { signal::ctrl_c().await.unwrap(); };
    #[cfg(unix)] let term = async { signal::unix::signal(signal::unix::SignalKind::terminate()).unwrap().recv().await; };
    #[cfg(not(unix))] let term = std::future::pending::<()>();
