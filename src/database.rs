...
impl Database {
    ...
    pub async fn test_query(&self) -> Result<String, Error> {
        let row = sqlx::query(
            r#"
            SELECT
                (SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='servers') as servers_ok,
                (SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='ping_results') as ping_ok,
        ...
        let servers_ok: i64 = row.try_get("servers_ok")?;
        let ping_ok: i64 = row.try_get("ping_ok")?;
        let admin_count: i64 = row.try_get("admin_count")?;

        Ok(format!(
            "db status: tables_ok={}, admin_users={}",
            (servers_ok == 1 && ping_ok == 1),
            admin_count,
        ))
    }
